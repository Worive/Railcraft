...
public class ClientEffectProxy extends CommonEffectProxy {
    ...
    public void doForceSpawn(DataInputStream data) throws IOException {
        if (!shouldSpawnParticle(true)) return;
    ...
    public void tuningEffect(TileEntity start, TileEntity dest) {
        ...
        if (rand.nextInt(2) == 0) {
            ...
            if (isGoggleAuraActive(GoggleAura.SIGNALLING)) colorProfile = RenderTESRSignals.ColorProfile.ASPECT;
    ...
    public void trailEffect(int startX, int startY, int startZ, TileEntity dest, long colorSeed) {
        if (!shouldSpawnParticle(false)) return;
        if (Minecraft.getMinecraft().thePlayer.getDistanceSq(startX, startY, startZ) > TRACKING_DISTANCE) return;
        if (rand.nextInt(3) == 0) {
    ...
    public void chunkLoaderEffect(World world, Object source, Set<ChunkCoordIntPair> chunks) {
        if (!isGoggleAuraActive(GoggleAura.ANCHOR)) return;
    ...
    private boolean shouldSpawnParticle(boolean canDisable) {
        ...
        if (!canDisable && particleSetting > 1) particleSetting = 1;
        if (particleSetting == 1 && MiscTools.RANDOM.nextInt(3) == 0) particleSetting = 2;
